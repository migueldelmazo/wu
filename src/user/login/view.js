    ...
    return (
      <div className='container'>
        <div className='row'>
          ...
          <div className='col-6'>
            <p>Id: {atom.model.get('user.id')}</p>
            <p>Email: {this.state.email}</p>
            <p>isTrue: {this.state.isTrue + ''}</p>
            <p>user.model.router: {atom.model.get('user.login.route.isValidPathName') + ''}</p>
            <p>Time: {Date.now()}</p>
          </div>
          ...
          <div className='col-6'>
            ...
            <button
              className='btn btn-primary'
              onClick={this.onEvent('setState', { email: 'info@migueldelmazo.com' })}
            >
              Set
            </button>
            <button
